...
# Fix the comparison window even when DELAY_LAGS=0 for a zero-lag ablation.
...
# DELAY_MODE=raw uses raw-history guidance; DELAY_LAGS=0 tests zero lag only.
...
for pred_len in "${horizons[@]}"; do
  ...
  printf '\nRunning itransformer_delay: pred_len=%s mode=%s lags=%s context=%s\n' \
    "$pred_len" "$DELAY_MODE" "$DELAY_LAGS" "$DELAY_CONTEXT_LEN"
  "$PYTHON" -u run.py \
    --is_training 1 \
    --root_path "$ROOT_PATH" \
    --data_path ETTh1.csv \
    --model_id "ETTh1_delay_96_${pred_len}_w256" \
